...
echo "<form action='..\LoggedMain.php'>
	...
	if($_SESSION['powerlevel'] >= 1){
		echo"<form align='right' action='NoviVestiDana.php'>
...
function getNewsToday($conn){
	...
	while($row = $result->fetch_assoc()){
		...
		echo $row['message']."<br>";
		if($_SESSION['powerlevel'] >= 1){
			echo"<form method='POST' action='".deleteVestiDana($conn)."' style='position:absolute;top:0px;right:0px;'>
				<input type='hidden' name='id' value='".$row['id']."'>
				<button type='submit' name='deleteVestiDana'>Delete</button>
			</form>";
		}
...
function deleteVestiDana($conn){
	if(isset($_POST['deleteVestiDana'])){	
		$id = $_POST['id'];	
